...
| May 19 | Rodrigo took over networking case | Scheduled callback |
| May 20 | Remote Kafka spikes to 140s | Gaurav identified batch idle + cross-VPC |
| May 21 | Gaurav recommended ParallelApplyBufferSize 500 | Customer applied |
| May 22 | Rodrigo confirmed network OK | Offered VPC Peering |
...
2. **LOB handling** — Customer has JSON columns. LobMaxSize reduced to 32KB may cause extra lookups back to source for large JSON values (Gaurav flagged this risk).
...
- [ ] Coordinate with Gaurav — DMS needs deeper investigation (task logs, SOURCE vs TARGET timing)
